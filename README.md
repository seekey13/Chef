...
- **Left-click** the icon for a menu of the usable items in your inventory with their stack counts, grouped as **Food**, **Drink**, **Combat Skill** and **Crafting Skill**; picking one runs `/item "<name>" <me>`. Hovering an entry shows its in-game description. **Open Store** at the bottom runs `!chef`. The menu closes on either.
